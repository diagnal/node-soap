...
PortTypeElement.prototype.postProcess = function(definitions) {
  ...
  for (var i = 0, child; child = children[i]; i++) {
    ...
    if(!this.methods.hasOwnProperty(child.$name)){
...
BindingElement.prototype.postProcess = function(definitions) {
  ...
  if (portType){
    ...
    for (var i = 0, child; child = children[i]; i++) {
      ...
      if(tempMethod === child.$name){
        continue;
      }
      tempMethod = child.$name;
...
ServiceElement.prototype.postProcess = function(definitions) {
  var children = this.children,
    ...
    //Deleting soap12 bindings
    var keys= Object.keys(bindings);
    delete bindings[keys[1]];
...
WSDL.prototype._initializeOptions = function (options) {
  ...
  //Added for namespace fix:Muralikrishnan #NS FIX
  this.options.unwantedNamespaces=options.unwantedNamespaces? options.unwantedNamespaces:[];
  this.options.addedNamespaces=options.addedNamespaces? options.addedNamespaces:{};
...
WSDL.prototype._xmlnsMap = function() {
  ...
  //Added for namespace fix:Muralikrishnan #NS FIX
  for(var key in this.options.addedNamespaces) xmlns[key]=this.options.addedNamespaces[key];

  var str = '';
  for (var alias in xmlns) {
    if (alias === '' || alias === TNS_PREFIX || (this.options.unwantedNamespaces.indexOf(alias) > -1) ) //#NS FIX
      continue;
    var ns = xmlns[alias];
    switch (ns) {
